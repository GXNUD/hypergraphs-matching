...
int main(int argc, const char *argv[]){
  ...
  GpuMat img1(imread("./536.jpg", 0));
  if (img1.data == NULL) {
		printf("cv::imread() failed...\n");
		return -1;
  }
  GpuMat img2(imread("./536.jpg", 0));
  if (img2.data == NULL) {
		printf("cv::imread() failed...\n");
		return -1;
  }
  ...
  /*cout << "valor primer punto" << keypoints1GPU.ptr<float>(0)[0]<<
    " "<<keypoints1GPU.ptr<float>(1)[0]<<endl;*/

  // extraccion de puntos caracteristicas con la gpu
  ...
  //                                                   descriptors2, 15, 10, 5,
  //                                                   0.85);
  // TODO Point matching

  // TODO Draw matching
  // drawEdgesMatch(img11, img22, matches, Edges1, Edges2, keypoints1, keypoints2);
